...
func (suite *OrderRepositoryTestSuite) Test_WhenList_ThenShouldReturnAnEmptyArrayOfOrders() {
	repo := NewOrderRepository(suite.Db)
...
func (suite *OrderRepositoryTestSuite) TestGivenAnOrder_WhenList_ThenShouldReturnAnArrayOfOrders() {
	order, err := entity.NewOrder("123", 10.0, 2.0)
	suite.NoError(err)
	suite.NoError(order.CalculateFinalPrice())
	repo := NewOrderRepository(suite.Db)
